...
metadata {
    definition (name: "HA Presence Sensor RSSI", namespace: "fison67", author: "iquixfison67") {
...
def setStatusMap(map){
    ...
    log.debug "StatusMap[${state.entity_id}] >> ${map.state} ${map.attr}"
    setEnv(map.state, map?.attributes?.rssi)
}

def setEnv(value, rssi) {
...
def callback(physicalgraph.device.HubResponse hubResponse){
    ...
    try {
        ...
        setEnv(jsonObj.state, jsonObj?.attributes?.rssi)
    } catch (e) {
